...
from ipmi.common.docker.helper import check_image_exists
from ipmi.common.logger import LoggerMixin
from ipmi.common.shell import create_cli_command, execute, execute_in_docker
from ipmi.fused_types import PathLike
from ipmi.reconstruction.cbct.binning import save_curve
...
class Reconstructor(ABC, LoggerMixin):
    def __init__(
        ...
        if self.use_docker:
            if not check_image_exists(defaults.DOCKER_IMAGE):
                raise ImageNotFound(
                    f"Docker image {defaults.DOCKER_IMAGE} not found. "
                    f"Get it here: {defaults.DOCKER_IMAGE_REPO}"
                )
