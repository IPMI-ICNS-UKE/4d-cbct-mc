...
if [ "$SESSIONEXISTS" = "" ]; then
  ...
  tmux send-keys -t 'worker 1' 'conda activate cbctmc' C-m "${CALL} --n-workers 3 --i-worker 1 --gpu 0" C-m

  tmux new-window -t $SESSION:2 -n 'worker 2/gpu 1'
  tmux send-keys -t 'worker 2' 'conda activate cbctmc' C-m "${CALL} --n-workers 3 --i-worker 2 --gpu 1" C-m
...
#  tmux send-keys -t 'worker 3' 'conda activate cbctmc' C-m "${CALL} --n-workers 5 --i-worker 3 --gpu 2" C-m
  ...
  tmux send-keys -t 'worker 4' 'conda activate cbctmc' C-m "${CALL} --n-workers 3 --i-worker 3 --gpu 4" C-m
