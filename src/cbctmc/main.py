...
def createSegmentation(path, seg_filename, path_ct_in, filename_ct_in, gpu_id: int = 0):
    ...
    os.system(
        f"CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES={gpu_id} TotalSegmentator -i "
        + path_ct_in
        + "/"
        + filename_ct_in
        + " -o "
...
def writeVoxel(img, img_np, img_seg, path, path_out, vox_filename, vox_air_filename):
    ...
    air_header += (
        str(img.GetSpacing()[0] / 10 * img.GetSize()[0])
        + " "
    ...
    header += (
        str(img.GetSpacing()[0] / 10)
        + " "
        + str(img.GetSpacing()[1] / 10)
        + " "
        + str(img.GetSpacing()[2] / 10)
        + "\n"
    )
...
def createNumpy(
    ...
    proj = np.array(proj)
    air = readDoseImage(
        sim_path + "/" + sim_air_filename, det_pixel_x_halffan, det_pixel_x
    )
...
def writeXML(path, geo_filename, src_to_iso, src_to_det, no, lat_displacement):
    ...
    for i in range(no):
        ...
        f.write("<Matrix>" + "\n")
        f.write(
            str(-np.cos(g_angle_d) * src_to_det + lat_displacement * np.sin(g_angle_d))
            + "\t"
            + "0"
            + "\t"
            + str(
                -src_to_det * np.sin(g_angle_d) - lat_displacement * np.cos(g_angle_d)
            )
            + "\t"
            + str(lat_displacement * src_to_iso)
            + "\n"
            + "0"
            + "\t"
            + str(-src_to_det)
            ...
            + "0"
            + "\n"
            + str(-np.sin(g_angle_d))
            + "\t"
            + "0"
            + "\t"
            + str(np.cos(g_angle_d))
            + "\t"
            + str(-src_to_iso)
            + "\n"
        )
...
def writeInputFile(
    ...
    text += (
        str(photons)
        + " # TOTAL NUMBER OF HISTORIES, OR SIMULATION TIME IN SECONDS IF VALUE < 100000\n"
    )

    text += (
        f"{random_seed}                      # RANDOM SEED (ranecu PRNG)\n0                               "
        ...
        + "/3rd_party/mcgpu/125kVp_0.89mmTi.spc"
        + "     # X-RAY ENERGY SPECTRUM FILE\n"
        + str(float(size_x / 20))
        + " "
        + str(float(size_y / 20) - src_to_iso / 10)
        + " "
        + str(float(size_z / 20))
        + " "
    ...
    text += (
        "\t     # NUMBER OF PROJECTIONS (beam must be perpendicular to Z axis, set to 1 for a single projection)\n"
        + str(360 / no)
        + "                # ANGLE BETWEEN PROJECTIONS [degrees] (360/num_projections for full CT)\n"
        + "0.0 5000.0              # ANGLES OF INTEREST (projections outside the input interval will be skipped)\n"
        + str(src_to_iso / 10)
        + "       # SOURCE-TO-ROTATION AXIS DISTANCE (rotation radius, axis parallel to Z)\n "
    ...
    if not air:
        text += (
            ...
            + "/"
            + filename
            + "_dose.dat             # OUTPUT VOXEL DOSE FILE NAME"
        )
    else:
        text += (
            "/output"
            + "/"
            + filename
            + "air_dose.dat          # OUTPUT VOXEL DOSE FILE NAME"
        )
    ...
    if not air:
        text += (
            "/3rd_party/mcgpu/MC-GPU_material_files/muscle_tissue_new__5-125keV.mcgpu"
            + "# 2nd MATERIAL FILE\n"
...
def runSimulation(path, gpu_id: int = 0):
    os.chdir(path)
    os.system(
        f"docker run --rm --gpus device={gpu_id} -v $(pwd)/input:/input -v $(pwd)/output:/output -u $(id -u):$(id -g) "
...
def runModel(process_path, np_filename):
    ...
    for batch in tqdm(batch_array(proj, batch_size=6)):
        low_photon = torch.as_tensor(
            batch, device=config["device"], dtype=torch.float32
        )
        ...
        speedup_sample += torch.distributions.Poisson(rate=mean_scat).sample()
...
@click.option(
    ...
    default=True,
    type=click.BOOL,
    help="If speed_up = True 4.8e8 photons are simulated and the "
...
@click.option(
    "--det_pix_x", default=512, help="Number of Detector-pixel in X-Direction"
)
@click.option(
    "--det_pix_y", default=384, help="Number of Detector-pixel in Y-Direction"
)
@click.option(
    "--lat_displacement",
    default=-160,
    help="If cbct is used in half fan mode, give lateral displacement",
)
@click.option(
    "--src_to_detector",
    default=1500,
    help="Distance between X-Ray source and detector in mm",
)
@click.option(
    "--src_to_iso",
    default=1000,
    help="Distance between X-Ray source and rotation center in mm",
)
@click.option(
    "--photons",
    default=2.4e9,
    help="Number of photons to Simulate for each projection, only valid if "
    "speed_up is set False.",
)
@click.option(
    "--force_rerun", default=False, help="Set force_rerun=True to redo everything"
)
@click.option(
    "--force_segment", default=False, help="Set force_segment=True to redo segmentation"
)
@click.option(
    "--force_create_object",
    default=False,
    help="Set force_create_object=True to redo the object",
)
@click.option(
    "--force_simulate", default=False, help="Set force_simulate=True to redo simulation"
)
@click.option(
    "--gpu_id", default=0, type=click.INT, help="PCI ID of GPU for MC simulation"
)
@click.option(
    "--random_seed", default=42, type=click.INT, help="Random seed for MC simulation"
)
def run(
    path_ct_in,
    filename_ct_in,
    ...
    if (
        not os.path.exists(seg_path + "/" + seg_filename)
        or force_segment
        or force_rerun
    ):
        createSegmentation(
            seg_path, seg_filename, path_ct_in, filename_ct_in, gpu_id=gpu_id
        )
    img_seg = sitk.ReadImage(seg_path + "/" + seg_filename)

    # Create Voxel Object
    if (
        not os.path.exists(vox_path + "/" + vox_filename)
        or force_create_object
        or force_rerun
    ):
        writeVoxel(
            img_ct, img_np, img_seg, vox_path, path_out, vox_filename, vox_air_filename
        )

    # Prepare and Run MC-GPU Simulation
    if (
        not os.path.exists(process_path + "/" + log_filename)
        or force_simulate
        or force_rerun
    ):
        shutil.copyfile(vox_path + "/" + vox_filename, input_path + "/" + vox_filename)
        shutil.copyfile(
            vox_path + "/" + vox_air_filename, input_path + "/" + vox_air_filename
        )
        if speed_up:
            photons = 4.8e8
        writeInputFile(
    ...
    npToNifti(
        path_out, process_path, out_filename, np_filename, np_air_filename, det_pix_size
    )
    writeXML(
        path_out, geo_filename, src_to_iso, src_to_detector, no_sim, lat_displacement
    )
    #############################################################
