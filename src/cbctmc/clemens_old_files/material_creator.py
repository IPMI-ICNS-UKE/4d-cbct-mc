...
def getf2(material, energy):
    ...
    x = 2 * 20.6074 * energy / ele_mass  # dimensionless x(E) used for form factors
...
def createMat(formula, name, path, roh, energy_max):
    ...
    freepath = np.concatenate(
        (
            np.concatenate(
                (
                    energy.reshape((int((energy_max - energy_min) / 5) + 1, 1)),
                    np.swapaxes(mFreePath, 0, 1),
                ),
                axis=1,
            ),
            getF2(formula, energy_max).reshape(
                (int((energy_max - energy_min) / 5) + 1, 1)
            ),
        ),
        axis=1,
    )
