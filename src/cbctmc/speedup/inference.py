...
from cbctmc.speedup.models import MCSpeedUpUNet
...
class MCSpeedup(ForwardPassMixin):
    ...
    def _execute(
        ...
        n_projections, height, width = low_photon.shape
        logger.debug(
            f"Execute speedup for {n_projections} projections, each of shape {(height, width)}"
        )
        ...
        logger.debug(f"Splitting into {n_batches} batches of size {batch_size}")
