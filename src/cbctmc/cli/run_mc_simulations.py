...
_DEFAULT_SEGMENTER_WEIGHTS = Path(
    pkg_resources.resource_filename("cbctmc", "assets/weights/segmenter/default.pth")
)

_DEFAULT_SPEEDUP_WEIGHTS = Path(
    pkg_resources.resource_filename("cbctmc", "assets/weights/speedup/default.pth")
)
...
def _reconstruct_mc_simulation(
    ...
    if reconstruct_4d:
        ...
        for method in ("cg4d", "rooster4d"):
            output_folder = simulation_folder / config_name / "reconstructions"
            output_filename = f"{method}_wpc{suffix}.mha"
            # check if already reconstructed
            if (output_folder / output_filename).exists():
                logger.info(
                    f"Reconstruction {output_filename} already exists. Skip reconstruction."
                )
                continue
...
def run(
    ...
    for config_name, config in CONFIGS.items():
        ...
        if is_4d:
            ...
            additional_run_kwargs["respiratory_signal_quantization"] = (
