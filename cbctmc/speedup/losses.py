...
def consistency_loss(mean: torch.Tensor, var: torch.Tensor, target: torch.Tensor):
    ...
    gaussian_nll_loss = F.gaussian_nll_loss(input=mean, target=target, var=var)
...
def eight_loss(
    mean: torch.Tensor, var: torch.Tensor, target: torch.Tensor, target2: torch.Tensor
):
    gaussian_nll_loss = 0.1 * F.gaussian_nll_loss(input=mean, target=target, var=var)
    # for i in range(8):
