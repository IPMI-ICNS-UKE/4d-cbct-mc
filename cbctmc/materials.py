...
def getf2(material, energy):
    ...
    x = (
        2 * 20.6074 * energy / (5.10998918 * 1e5)
...
def createMat(formula, name, roh):
    ...
    with open("materialfiles/" + name + "_new__5-125keV.mcgpu", "w") as f:
        f.write(
            "#[MATERIAL DEFINITION FOR MC-GPU: interaction mean free path and sampling data from PENELOPE 2006]\n"
        )
        ...
        f.write(
            "#[Energy (eV)     | Rayleigh        | Compton         | Photoelectric   | TOTAL (+pair prod) (cm) |"
            " Rayleigh: max cumul prob F^2]\n"
        )
        freepath = np.concatenate(
            (
                np.concatenate(
                    (energy.reshape((24001, 1)), np.swapaxes(mFreePath, 0, 1)), axis=1
                ),
                getF2(formula).reshape((24001, 1)),
            ),
            axis=1,
        )
        ...
        np.savetxt(
            f, np.concatenate((rita, lim), axis=1), fmt="%.10e %.10e %.10e %.10e %i %i"
        )
        f.write(
            "#[COMPTON INTERACTIONS (relativistic impulse model with approximated "
